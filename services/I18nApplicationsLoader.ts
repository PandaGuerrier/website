...
import { LoaderContract } from '@ioc:Adonis/Addons/I18n'

export type ApplicationLoaderOptions = {
  locations: string[];
};
 ...
 * files
 */
export class I18nApplicationsLoader implements LoaderContract {
  constructor(private config: ApplicationLoaderOptions) {}
   ...
   */
  private getFileContents(location: string, filePath: string): Promise<string> {
    return new Promise((resolve, reject) => {
      readFile(join(location, filePath), 'utf-8', (error, contents) => {
        if (error) {
          reject(error)
          return
        }
   ...
   */
  private processFileTranslations(filePath: string, messages: Record<string, string>, messagesBag: Record<string, any>) {
   ...
   */
  private async processJSONFile(location: string, filePath: string, messagesBag: Record<string, any>) {
    const contents = await this.getFileContents(location, filePath)
   ...
   */
  private async processYamlFile(location: string, filePath: string, messagesBag: Record<string, any>) {
    const contents = await this.getFileContents(location, filePath)
   ...
   */
  private getTranslationFiles(location: string) {
    return fsReadAll(
      location,
  ...
  public async load() {
    ...
    await Promise.all(
      this.config.locations.flatMap((location) => this.getTranslationFiles(location).map((file) => {
        if (file.endsWith('.json')) {
          return this.processJSONFile(location, file, messagesBag)
        } else {
          return this.processYamlFile(location, file, messagesBag)
        }
      }))
